...
It enumerates every REST and SOAP family the server hosts, per tenant and per version, and links each one to a **`/help` page** (`{base}/{family}/help`) describing its operations. Reaching it through a browser requires the **Access to SOA Admin Page** application-security setting (see [Authentication](00-Authentication.md#application-security-settings-that-affect-api-access)); a bearer token works too — [see below](#it-takes-a-bearer-token-so-read-it-in-code-not-in-a-browser).
...
#### It takes a Bearer token — so read it in code, not in a browser

`apiref.aspx` is an ASPX page, which makes it look like a browser-only surface. It is not: **the same `Authorization: Bearer {token}` you use for every other call authenticates it**, so the family list is scriptable. Unauthenticated it **302s to `/docs/logon.aspx`** and serves a login form with HTTP 200 — so a client that follows redirects and checks only the status code will parse the login page and conclude the tenant hosts no APIs. Assert on content, not on `200`.

Verified on 26.1.5950.0 (September 2026). The page also prints the middleware version, SQL data source and database name in its header, which makes it a quick way to confirm **which environment you are actually pointed at** before running anything.

#### What it publishes — one tenant, 26.1.5950.0

**48 families**, each linked to its own `/help`. This is one tenant at one version — a worked example of the method, not a universal list:

| Group | Families |
|---|---|
| **Entity** | `api/entity/addresses` · `api/entity/contacts` · `api/entity/customers` · `api/entity/vendors` |
| **Inventory** | `api/inventory/externalcounts` · `api/inventory/inventoryadjustments` · `api/inventory/inventorymovement` · `api/inventory/parts` · `api/inventory/partscan` · `api/inventory/rental` · `api/inventory/serialnumberextdinfo` · `api/inventory/v2/parts` |
| **Sales / purchasing / service** | `api/sales/consignmentusageorders` · `api/sales/invoices` · `api/sales/opportunities` · `api/sales/orders` · `api/sales/tasks` · `api/purchasing/purchaseorders` · `api/service/serviceorders` |
| **Accounting / payments** | `api/accounting/customerformtemplates` · `api/accounting/exchangerates` · `api/accounting/gl` · `api/epayments` · `api/epayments-legacy` · `api/cardstorage` |
...
#### Probe with `/help`, not `/ping` — a `/ping` 404 proves nothing

**Correction (September 2026).** This section previously carried a sweep built by calling `GET {base}/{family}/ping` against a guessed list of names, split into "answering" and "not on this tenant". **The right-hand column was wrong.** Re-run against `apiref.aspx`'s own list on 26.1.5950.0, **every family that sweep wrote off is published and answers `/help` with HTTP 200**:

| Family | `GET /ping` | `GET /help` |
|---|---|---|
| `sales/invoices`, `chat`, `ecommerce`, `eh`, `environment/systems`, `localization`, `printing`, `inventory/rental`, `inventory/inventorymovement`, `integrationProcedures`, `.configuration`, `pathguide`, `help` | **404** | **200** |
| `document`, `logistics/roadnet` | **500** | **200** |
| `cardstorage` | **405** | **200** |

`ping` is an ordinary route that a family either implements or doesn't — it is not a health check the middleware answers on every family's behalf. So a `/ping` 404 measures the absence of a `ping` handler, not the absence of the API. `sales/invoices` is the one that makes the cost concrete: it was published on the tenant the whole time, and the sweep recorded it as missing.

**Use the list, then `/help`.** Read family names off `apiref.aspx` rather than guessing them, and confirm a family with **`GET {base}/{family}/help`**. Probing guessed names with `/ping` produces false negatives in both directions — it is how the earlier `purchasing/*` claim and this table both went wrong.

#### The other two catalogs

`apiref.aspx` lists REST and SOAP **families**. Two more endpoints enumerate the surfaces those families don't cover, both taking the same bearer token:

| Catalog | Returns | On the 26.1.5950.0 tenant |
|---|---|---|
| `GET {uiserver}/api/v2/services` | Transaction API service names | **301** (`m_*` services, reports and `m_storedprocedureexecutor` are [hidden but still callable](03-Transaction-API.md#endpoints)) |
| `GET {base}/odataservice/odata/table/` and `/view/` | OData service document — every exposed object, names only | **3,407** tables, **3,758** views |

The OData service document is the cheap half of [`$metadata`](02-OData-API.md#discovering-whats-exposed-metadata): names only, no columns. Reach for it when you need to answer "does this table exist here" and don't want the full schema.
