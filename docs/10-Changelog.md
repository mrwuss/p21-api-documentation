...
- **feat(site):** **Per-section "Copy HTML" buttons** on the Changelog (each dated release) and Breaking Changes (each version section *and* each individual entry) — for quoting a release or a single finding in a forum post. Hovering a heading reveals the button; it copies **that section only**, stopping cleanly at the next heading of equal or higher rank. Every link is rewritten to an **absolute URL** so it still resolves once pasted — including same-page anchors, and resolved against the page's canonical published URL rather than `document.baseURI` (which would bake in a `file://` path when the page is opened locally). Syntax-highlight `<span>`s are flattened to plain `<pre><code>` (their classes mean nothing without this site's stylesheet) and page-local `id`s are stripped so they can't collide with the host page. Copied as both `text/html` and `text/plain`, so it pastes rendered into a rich editor and as markup into a source view, with an `execCommand` fallback for non-secure contexts — *@mrwuss*
