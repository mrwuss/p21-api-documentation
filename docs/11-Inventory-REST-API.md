...
Note it is an **HTTP 500**, not a 400 — a missing query parameter surfaces as a server error here, so do not filter these out of your logs as transient.

Three further query parameters are optional: **`shipToId`, `orderDate` and `jobNo`** — all three accepted without error on 26.1.5940.0. Note the names: the WCF *method* parameters are `optionalShipToId` / `optionalOrderDate` / `optionalJobNo` (that is how they appear in a stack trace), but the **query string** spells them without the `optional` prefix. The tenant's own help page gives the authoritative template:

```text
GET {base}/api/inventory/parts/help/operations/GetItemPrices
→ .../parts/prices?companyId={}&customerId={}&salesLocId={}&shipToId={}&orderDate={}&jobNo={}
```

That help page is also where the XML request/response schemas come from, and it is per-tenant — prefer it over this table if the two ever disagree.
...
| `application/json` | XML | **400** — at least a real P21 error: `Invalid JSON or request body for parameter 'itemInfo'. Unexpected character encountered while parsing value: <...` |

`Accept` is negotiated independently of `Content-Type`: an XML body with `Accept: application/json` returns JSON, and vice versa. All of the above re-verified on **26.1.5940.0**.

> **The XML body takes no namespace, and getting that wrong looks exactly like "XML is rejected".** `<ArrayOfItemPriceInfo>` must be bare — as the tenant's own `.../parts/help/operations/GetItemPrices` page shows it. Adding a plausible DataContract namespace (`http://schemas.datacontract.org/2004/07/P21.Inventory.Model`, the shape used elsewhere in this API) makes the element fail to bind and returns the **same generic WCF 400 HTML page** as the header mismatch above. Two different mistakes, one indistinguishable error — which is most of why this endpoint has its reputation.
