...
**Upserts (July 2026)**: keyed `Status: "New"` List rows are an upsert — update when the key matches, **insert a new row when it doesn't** (verified: 81 new JobContractPricing lines in one run, DB-confirmed; credit Alex Westemeier). Gotchas: order `pricing_method` before `price` in Edits (cascade silently zeroes the price); one transaction per POST when inserts re-save a shared FORM header (optimistic-concurrency collisions + duplicate `line_no`); header saves validate `end_date` ≥ today. `IgnoreDisabled: true` (payload top level ONLY — silently ignored inside a Transaction object) unlocks disabled columns and tabs, e.g. contract BINS quantities and JOBPRICECOST commission fields. **Boundary (26.1, Aug 2026):** on `JobContractPricing` `VALUES.values`, `IgnoreDisabled` does NOT unlock the tab, on any path — updating an existing line, inserting a new line, or creating a brand-new contract all refuse identically with `Tab page is disabled`; `IgnoreDisabled` just converts that into `Succeeded: 1` / `Status: "Passed"` with nothing written. Control: the same create with the `VALUES` DataElement omitted succeeds normally, so it's specifically `VALUES.values` that's refused, not contract/line creation. Always read back. See [docs/14](docs/14-Breaking-Changes.md#8-ignoredisabled-true-reports-success-on-jobcontractpricing-valuesvalues-writes-that-write-nothing).
