...
Usage:
    python examples/python/recipes/production_order_runbook.py            # checklist + dry run
    python examples/python/recipes/production_order_runbook.py --execute  # run Stage 3 + read-back
"""
...
Stage 1 - Create the production order
  Path A: sales order auto-create (make-to-order) -- enter the assembly line
          via the Interactive API (examples/python/recipes/order_with_assembly.py).
...
Stage 2 - Log labor BEFORE printing (examples/python/recipes/record_labor_time.py)
...
Stage 7 - Fix quantity fallout (examples/python/recipes/inventory_adjustment.py)
